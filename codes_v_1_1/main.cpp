...
void init_rule_set_fun(QFile* pos_file)
{
    ...
    init_set_rules.append("{:%引用便签%:}{% note quote:: _文字 %}");
    init_set_rules.append("{:%信息便签%:}{% note info:: _文字 %}");
    init_set_rules.append("{:%警告便签%:}{% note warning:: _文字 %}");
    init_set_rules.append("{:%注意危险%:}{% note danger:: _文字 %}");
    init_set_rules.append("{:%成功%:}{% note success:: _文字 %}");
    init_set_rules.append("{:%bug提示%:}{% note bug red:: _文字 %}");
    init_set_rules.append("{:%计划便签%:}{% note todo:: _文字 %}");
    init_set_rules.append("{:%引导%:}{% note guide clear:: _文字 %}");
    init_set_rules.append("{:%下载便签%:}{% note download:: _文字 %}");
    init_set_rules.append("{:%信息%:}{% note message gray:: _文字 %}");
    init_set_rules.append("{:%更新%:}{% note up:: _文字 %}");
    init_set_rules.append("{:%撤回%:}{% note undo light:: _文字 %}");
...
int main(int argc, char *argv[])
{
    ...
    QFontDatabase::addApplicationFont(":/res/fonts/JJ.ttf");
